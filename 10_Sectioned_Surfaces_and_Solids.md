...
**Independent BasisCurve.** When a guide curve's `BasisCurve` is an independent curve it carries geometric shape not constrained to be linear relative to the `Directrix` distance parameter. This is the configuration closest to the industry concept of a string line: an edge path defined by its own geometry rather than by linearly interpolated offsets from a centerline. Because an independent `BasisCurve` has its own distance parameterization, there is no shared axis along which to scope tag matching; tags must be treated as globally unique across the entire model to avoid ambiguity. The trade-offs of this approach are illustrated in §10.6.1.6.

The IFC specification does not state that tags must be unique within any particular scope, and there currently is no machine-verifiable rule to enforce either uniqueness constraint. Without clarity on the `BasisCurve` requirement and tag scoping rules, differing interpretations across implementations will produce incompatible results and undermine the ability to reliably exchange string-line-based geometry. The specification must choose one of these two options — a model cannot combine both — so an implementation agreement is necessary to resolve which applies and to allow developers to confidently implement sectioned surfaces and solids.
...
#### 10.6.1.5 String Lines — Guide Curves as Alignments
...
*Figure 10.6.1.6-2 — Expected geometry of `IfcSectionedSurface_with_stringlines_independent_edge_alignments.ifc`. This rendering has been synthesized to illustrate the correct geometry; no viewer capable of resolving string line guide curves has been found at the time of writing.*
...
#### 10.6.2.5 String Lines — Guide Curves as Alignments
...
#### 10.6.2.6 String Lines — Independent Edge Alignments
