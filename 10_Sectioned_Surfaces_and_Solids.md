...
The file [`IfcSectionedSurface.ifc`](examples/IfcSectionedSurface.ifc) is the surface counterpart to [`IfcSectionedSolidHorizontal.ifc`](examples/IfcSectionedSolidHorizontal.ifc) (§10.6.2.1). It demonstrates `IfcSectionedSurface` with an `IfcSegmentedReferenceCurve` as the `Directrix`, so the open cross-section profile rotates with cant at each station along the alignment. The alignment geometry is identical to the solid example.

The alignment comprises three stacked layouts:

- **Horizontal**: a 100 m Bloss transition from infinite radius (tangent entry) to a 300 m radius curve curving left.
...
The horizontal and vertical layouts combine into an `IfcGradientCurve`; adding the cant layout produces an `IfcSegmentedReferenceCurve`, which serves as the `Directrix`. The cross-section is a single `IfcOpenCrossProfileDef` instance shared at both positions: two segments of 29.5 m half-width each, both with zero slope. The same profile instance is placed at distances 0 m and 100 m along the directrix.

Because the profile slope is zero, the top surface is flat in the profile's local coordinate frame at every station. The banking visible in the rendered model comes entirely from the `IfcSegmentedReferenceCurve` rotating the profile frame at each station by the cant-derived angle. At the start (distance 0) the cant is zero and the surface is level; by distance 100 m the cant has reached 160 mm and the surface is banked accordingly. The geometry engine interpolates the cant continuously between stations following the Bloss parametric equations — there is no step change.

**Note on IfcOpenShell rendering.** TODO item #22 records that the current IfcOpenShell `IfcSectionedSolidHorizontal` implementation does not rotate the solid cross-section based on cant. The same limitation applies to `IfcSectionedSurface`: the rendered surface in IfcOpenShell does not reflect the cant banking. The IFC model itself is correctly structured — the `IfcSegmentedReferenceCurve` carries the cant — but the geometry kernel does not yet apply the rotation.
...
![](images/Figure_10.6.1.7-1_Dense_Section_Approximation.png)
...
![](images/Figure_10.6.1.7-2_Edge_Approximation_Gap.png)
