...
Road design software has long addressed this limitation through the concept of *strings* — continuous three-dimensional threads tracing significant features along the route: the edge of pavement, the shoulder break, the top of curb, the back of curb. Rather than deriving edges from template interpolation, strings define the edges explicitly. The cross-sections through the design are understood as sections through the string model, not the primary definition.

IFC4x3 introduces two geometry entities specifically for infrastructure: `IfcSectionedSurface` and `IfcSectionedSolidHorizontal`. Both define geometry by sweeping cross-sections along a `Directrix` — typically an alignment curve. `IfcSectionedSurface` uses open cross-sections to produce a surface; `IfcSectionedSolidHorizontal` uses closed cross-sections to produce a volumetric solid. Both support two complementary approaches to defining the geometry: template-based (cross-sections at defined distances along the directrix, linearly interpolated between) and string-line-based (guide curves that control where tagged cross-section points travel between sections). The IFC specification leaves several aspects of these entities underspecified — particularly around guide curve behavior, tag scoping, and the interaction between authored sections and guide curves. These gaps are significant enough that consistent implementations cannot be guaranteed without additional guidance; they are identified and discussed in §10.5.
...
The string line approach is not an alternative to the template approach but an augmentation of it. Cross-sections are always required; guide curves control interpolation between them.
...
In the context of `IfcSectionedSurface`, a breakline is a line along the surface where the cross-slope changes abruptly — an edge of pavement, a shoulder break, the face of a curb. This usage is distinct from DTM breaklines, which force TIN triangulation to honor a feature edge. Here the term describes a topological feature of the swept surface itself: a ridge or valley line where adjacent surface facets meet at a non-tangent angle.
...
The tag mechanism serves both breaklines and string lines simultaneously. A tag on a profile vertex identifies that vertex's correspondence across sections with different topology and, when a matching `IfcOffsetCurveByDistances` guide curve exists, controls the vertex's trajectory between sections. The two functions — topological correspondence and geometric guidance — share the same tag infrastructure.

### 10.3.2 String Lines

Within `IfcSectionedSurface`, string lines are implemented through the `Tags` attribute of `IfcOpenCrossProfileDef`. This attribute holds a list of $N+1$ labels — one per vertex of the open profile, where $N$ is the number of segments. An `IfcOffsetCurveByDistances` guide curve whose `Tag` matches one of these vertex labels takes control of that vertex's trajectory along the surface.

The `OffsetPoint` attribute of `IfcOpenCrossProfileDef` positions the first tagged vertex in the directrix's local coordinate system. Subsequent vertices are derived from there by accumulating the segment `Widths` and `Slopes`. It is unclear whether all tagged vertices participate in the matching process independently, allowing a surface to have some vertices guided by offset curves and others governed by linear interpolation between authored sections (see §10.5.2).
...
As with `IfcSectionedSurface`, the solid is generated by sweeping only between the defined `CrossSectionPositions`. It does not extend to the head or tail of the `Directrix` (see §10.5.3). The `CrossSectionPositions` and `CrossSections` lists must be equal in length, and the position expressions must not use longitudinal offsets.

Cross-sections are typically `IfcArbitraryClosedProfileDef` profiles. The profile outline is defined as an `IfcIndexedPolyCurve` referencing an `IfcCartesianPointList2D`. Profile points are listed in counter-clockwise order when viewed from the direction the profile normal points. The `IfcCartesianPointList2D.TagList` attribute assigns a label to each point in the coordinate list, enabling the string line mechanism of §10.2. The same two `BasisCurve`-driven scoping behaviors available for `IfcSectionedSurface` — Local Scoping and Global Scoping — apply equally here: guide curves whose `BasisCurve` is the same curve as the solid `Directrix` are scoped to that directrix and their tags need only be unique within that scope; guide curves with an independent `BasisCurve` require globally unique tags across the model. The trade-offs between these approaches are discussed through `IfcSectionedSurface` examples in §10.6.1.5 and §10.6.1.6, but the analysis applies without change to `IfcSectionedSolidHorizontal`.

For a note on a documentation error in the profile orientation specification for this entity, see §10.5.6.
...
### 10.5.1 Guide Curve BasisCurve and Tag Scoping

The IFC specification does not require or restrict the `BasisCurve` of an `IfcOffsetCurveByDistances` guide curve to be the same curve as the `Directrix` of the surface or solid it serves. This silence permits two distinct approaches to modeling sectioned surfaces and solids, each with different implications for tag scoping.

**Local Scoping.** When a guide curve's `BasisCurve` is the same curve as the `Directrix`, the guide curve's offsets are expressed in the same distance parameterization as the `CrossSectionPositions`. This provides a natural tag scoping boundary: only guide curves sharing that `BasisCurve` are candidates for the surface or solid's tag matching, and tags need only be unique within that scope. However, `IfcOffsetCurveByDistances` interpolates lateral offsets linearly between its defined `IfcPointByDistanceExpression` entries. Because the guide curve and the cross-section template share the same linear-along-the-directrix model, this approach produces results geometrically equivalent to placing cross-sections at the guide curve's defined distances — no geometric advantage over the pure template approach is gained.

**Global Scoping.** When a guide curve's `BasisCurve` is an independent curve, it carries geometric shape not constrained to be linear relative to the `Directrix` distance parameter. This is the configuration closest to the industry concept of a string line: an edge path defined by its own geometry rather than by linearly interpolated offsets from a centerline. Because an independent `BasisCurve` has its own distance parameterization, there is no shared axis along which to scope tag matching; tags must be treated as globally unique across the entire model to avoid ambiguity.

**Representation Scoping.** A third candidate scoping rule avoids Global Scoping's requirement for model-wide unique tags by relying on a structural mechanism the IFC file format already provides: representation containment. The guide curves are restricted to a separate `IfcShapeRepresentation` that is a sibling entry within the same `IfcProductDefinitionShape.Representations` list as the representation containing the surface or solid. This has the useful side effect of also satisfying the rooting requirement of §10.5.7 without a dedicated placeholder host entity. Tags need then be unique only within this local representation scope, independent of whether the guide curves' `BasisCurve` matches the `Directrix` — resolving Global Scoping's uniqueness burden without requiring the guide curve to share the directrix's parameterization. This candidate does not fully close the gap on its own: if a model author places more than one `IfcSectionedSurface` across the representations of a single `IfcProductDefinitionShape` — which is schema-legal — representation-scoping alone cannot determine which guide curves belong to which surface, and the ambiguity this chapter has documented resurfaces one level up.

All three of these approaches to tag scoping **have no basis in the specification text.** The IFC specification does not state that tags must be unique within any particular scope, and there currently is no machine-verifiable rule to enforce any of the three candidate scoping mechanisms described above. Without clarity on tag scoping rules, differing interpretations across implementations will produce incompatible results and undermine the ability to reliably exchange string-line-based geometry. An implementation agreement is necessary to designate an authoritative scoping rule, or an authoritative precedence among them, so developers can confidently implement sectioned surfaces and solids.

**Authoring Versus Consuming.** The three scoping techniques above are described from the perspective of an authoring or exporting application, which controls how tags are assigned and can guarantee whichever uniqueness discipline it chooses. Reading such a model back imposes a much harder burden. No IFC attribute, inverse relationship, or STEP entity identifier points from a tagged vertex in an `IfcSectionedSurface` or `IfcSectionedSolidHorizontal` profile to the guide curve that controls it — the only link is the shared string value of `Tag`. Resolving a guided vertex therefore requires a global search of the entire model for every `IfcOffsetCurveByDistances` instance carrying a matching `Tag`, and only then does a scoping rule help, by narrowing which of the matches found in that search is the correct one. Scoping resolves ambiguity among candidates; it does not remove the need to find them. Without a more direct reference mechanism, efficiently importing, interpreting, and rendering sectioned surfaces and solids defined by string lines is extremely difficult, and may not be practically achievable at all.

### 10.5.2 Partial Guide Curve Coverage

The IFC specification does not define what happens when a tagged cross-section vertex has no corresponding guide curve. The ambiguity is meaningful: if a tagged vertex with no matching guide curve falls back to linear interpolation between authored sections, then guide curves and linear interpolation can be freely mixed within the same surface or solid — some vertices precisely guided, others interpolated. If instead every tagged vertex is required to have a matching guide curve, models must supply redundant zero-offset guide curves for vertices whose trajectory is already linear (as in the centerline vertex of §10.6.1.4 and §10.6.2.5). The practical use case for mixing is clear: exterior edge vertices follow string line guide curves through a curved widening while interior breakline vertices use linear interpolation — combining the geometric precision of string lines with the simplicity of template-based sections. An implementation agreement is needed to confirm that a tagged vertex with no matching guide curve is treated as unguided and linearly interpolated, rather than as a modeling error.

### 10.5.3 Extent Along the Directrix
...
Guide curve extent follows the opposite rule. The `IfcOffsetCurveByDistances` specification states that if the defined offset values do not span the full extent of the basis curve, the offsets implicitly continue with the same value toward the head and tail. For guide curves this means a constant offset string line — such as the edge of a uniform-width shoulder — can be expressed with a single `IfcPointByDistanceExpression` without requiring offset values at every cross-section position. The two behaviors are independent: guide curves extend implicitly because they control interpolation *within* the surface or solid's defined span, not because they extend that span.

### 10.5.4 Disagreement Between Section Endpoint and Guide Curve

When a cross-section vertex is authored at a specific position and a guide curve with a matching tag passes through a different position for the same cross-section, the specification does not provide guidance on which governs. This ambiguity leads to interoperability issues including visualization, surface area and volume estimates, and clash detection.

This disagreement is possible because a guided vertex's position is defined twice, through two independent mechanisms: once by the profile's authored coordinate, and once by the string line's own geometry. The specification does not state that these two descriptions must agree, nor does it identify one as authoritative when they diverge. The example in §10.6.1.6 illustrates this redundancy directly.

The redundancy also leaves an unanswered modeling question: whether an authored coordinate for a guided vertex must be geometrically consistent with the string line at that position, or whether it may serve only as a placeholder because the string line is understood to override it. An official implementation agreement is needed to resolve which value governs and to confirm whether an inexact coordinate on a guided vertex is a modeling error or an accepted convention.

### 10.5.5 Cross-Section Orientation: Default Axis Direction
...
On alignments with cant, the Axis = (0,0,1) interpretation would exclude the cross-section rotation from the sweep, requiring it to be explicitly constructed into the `IfcSectionedSurface` profile or the `IfcSectionedSolidHorizontal` section rotation mechanism. The other interpretation would allow for cant-defined rotation to occur by simply using the default axis direction, as discussed in §10.6.1.1 and §10.6.2.1.

Figures 10.5.5-1 and 10.5.5-2 show the retaining wall example of §10.6.2.4 swept along a 10% graded directrix. In Figure 10.5.5-1, `Axis` is supplied explicitly as `(0, 0, 1)` at each cross-section position and the end face is plumb. Figure 10.5.5-2 shows the same scenario with `Axis` omitted, rendered by two different IFC viewers that have adopted opposite defaults: the upper half uses the perpendicular-to-3D-tangent interpretation; the lower half uses `(0, 0, 1)`. The divergence is most apparent at the start and end of the wall, where the two viewers produce noticeably different face orientations from an identical input file.

![Figure 10.5.5-1 — Elevation view of IfcSectionedSolidHorizontal on a 10% graded alignment with Axis = (0,0,1): end face is vertical.](images/Figure_10.5.5-1_Axis_GlobalZ.png)

*Figure 10.5.5-1 — Elevation view of `IfcSectionedSolidHorizontal` on a 10% graded alignment with `Axis = (0, 0, 1)`: end face is vertical (plumb).*

![Figure 10.5.5-2 — The same IfcSectionedSolidHorizontal model (retaining wall, 10% grade, Axis omitted) rendered by two IFC viewers: upper half assumes Axis perpendicular to the 3D tangent (inclined end face); lower half assumes Axis = (0,0,1) (plumb end face).](images/Figure_10.5.5-2_Axis_Tilted.png)

*Figure 10.5.5-2 — The same `IfcSectionedSolidHorizontal` model (retaining wall, 10% grade, `Axis` omitted) rendered by two IFC viewers: the upper half assumes `Axis` perpendicular to the 3D tangent (inclined end face); the lower half assumes `Axis = (0, 0, 1)` (plumb end face).*
...
**Recommendation.** Until such time as this discrepancy can be resolved, authors should always supply `Axis` explicitly in every `IfcAxis2PlacementLinear` entry within `CrossSectionPositions`. While this is onerous and redundant for cant alignments, it will accurately convey the modeling intent.

### 10.5.6 Profile Orientation Documentation Error
...
### 10.5.7 Validation Service: IfcOffsetCurveByDistances Must Be Referenced by a Rooted Entity

As discussed in [§5.2](5_OffsetCurves.md#52-offset-curves-and-ifcalignment), `IfcOffsetCurveByDistances` is a resource entity and must be directly associated with a rooted entity to satisfy validation rule [IFC105](https://buildingsmart.github.io/ifc-gherkin-rules/branches/main/features/IFC105_Resource-entities-need-to-be-referenced-by-rooted-entity.html). This requirement has an extra complication in the guide curve context: although a guide curve is referenced through to the surface or solid (itself a rooted entity) by way of the `Tag` mechanism described in §10.2, the validation service does not trace that path and flags guide curves not directly associated with a rooted entity.

Some techniques satisfy this requirement, falling into two categories: creating a placeholder host product for the guide curve to represent, or attaching the guide curve to a representation that already exists.

**Placeholder host product.** The guide curve is assigned as the `Representation` of some other rooted `IfcProduct`, created solely to give the guide curve a home. Any product subtype will satisfy IFC105 this way.

*`IfcAlignment`.* This is the lowest-overhead choice: `IfcAlignment` is a lightweight rooted entity, and aggregation (`IfcRelAggregates`) to `IfcProject` is the only additional relationship required. Its drawback is that a placeholder `IfcAlignment` is indistinguishable in the model from a real, designed alignment.

*`IfcAnnotation`.* An `IfcAnnotation` carries no implication of being alignment design content — it reads unambiguously as non-physical, informational geometry — but at greater overhead than `IfcAlignment`: per the IFC4x3 specification, an [`IfcAnnotation`](https://ifc43-docs.standards.buildingsmart.org/IFC/RELEASE/IFC4x3/HTML/lexical/IfcAnnotation.htm) must be related to a spatial structure element through `IfcRelContainedInSpatialStructure`. Each placeholder therefore requires both a spatial container and the containment relationship to it.

**Sibling representation.** A different class of technique avoids creating any placeholder host product at all. The guide curves for a given surface or solid are placed as `Items` in a second `IfcShapeRepresentation` — its own `RepresentationIdentifier` (e.g. `"Axis"`) and a `RepresentationType` suited to curve geometry (e.g. `"GeometricCurveSet"`) — that is a sibling entry, alongside the representation holding the surface or solid, within the same `IfcProductDefinitionShape.Representations` list. Because that `IfcProductDefinitionShape` is already reachable only through the rooted `IfcProduct` that owns it (an `IfcPavement`, in this chapter's examples), every item in every one of its representations — including the sibling curve representation — inherits that same traceable path to a rooted entity, without a placeholder product of any kind. Its limitation is architectural rather than administrative: it depends on the guide curves belonging to a single host product with a single `IfcProductDefinitionShape`, and it stops working cleanly if that `IfcProductDefinitionShape` hosts more than one `IfcSectionedSurface` or `IfcSectionedSolidHorizontal` — see Representation Scoping in §10.5.1.

These techniques **have no basis in the specification text.** The IFC specification states only that resource entities must be reachable from a rooted entity, not how; it neither prescribes a placeholder host product nor describes the sibling-representation technique. Without an authoritative resolution, differing interpretations across implementations will produce incompatible results and undermine confidence that a given guide curve model will pass validation consistently. An implementation agreement is necessary to designate a recommended technique — or to confirm that validation and authoring tools must accept all of them — so developers can confidently implement guide curves that satisfy IFC105.
...
#### 10.6.1.4 String Lines — Guide Curves as Resource Entities

The file [`IfcSectionedSurface_with_stringlines_as_resource_entities.ifc`](examples/IfcSectionedSurface_with_stringlines_as_resource_entities.ifc) is the baseline string line example. The directrix is a 200 m straight alignment with a flat vertical profile. The symmetric open profile has two segments and three tagged vertices: `A` (right edge), `B` (centerline), and `C` (left edge), each with a corresponding `IfcOffsetCurveByDistances` guide curve sharing the same basis curve as the surface directrix.
...
> **Note:** The alignment (`Main-Line`) cannot itself serve as the guide curve for the centerline vertex: `IfcAlignment` does not have a `Tag` attribute. Guide curve B is therefore an `IfcOffsetCurveByDistances` with a zero lateral offset from the alignment, existing solely to carry the tag `"B"` on behalf of the alignment — though this may not be required if IFC allows a mix of guide curves and linear interpolation to define a sectioned surface or solid. This ambiguity is discussed in §10.5.2.

Figure 10.6.1.4-1 shows the expected geometry of this example.

![Figure 10.6.1.4-1 — Synthesized 3D render of an IfcSectionedSurface where two independent edge string line guide curves, defining curved left and right edges. The surface interpolates between cross-sections while the edges follow the guide curve geometry exactly.](images/Figure_10.6.1.4_Stringline_Guide_Curves.png)

*Figure 10.6.1.4-1 — Expected geometry of `IfcSectionedSurface` with string line guide curves. This rendering has been synthesized to illustrate the correct geometry; no viewer capable of resolving string line guide curves has been found at the time of writing.*

> **Note:** The three `IfcOffsetCurveByDistances` instances in this model are bare resource entities: they are referenced by the surface but not assigned as representations of any rooted entity. The IFC validation service flags this as an [IFC105](https://buildingsmart.github.io/ifc-gherkin-rules/branches/main/features/IFC105_Resource-entities-need-to-be-referenced-by-rooted-entity.html) violation. The model is geometrically correct but does not conform to industry best-practice validation rules.

#### 10.6.1.5 String Lines — Guide Curves as Representations

The file [`IfcSectionedSurface_with_stringlines_guide_curves_as_alignments.ifc`](examples/IfcSectionedSurface_with_stringlines_guide_curves_as_alignments.ifc) contains the same geometry as §10.6.1.4 but wraps each guide curve as an `IfcAlignment` instance — `A-line`, `B-line`, and `C-line` — aggregated under the project. The `Tag` is set on the `IfcOffsetCurveByDistances` carried by each alignment's shape representation. This is the placeholder-`IfcAlignment` rooting technique of §10.5.7 — one of two rooting techniques this chapter demonstrates. This structure satisfies the [IFC105](https://buildingsmart.github.io/ifc-gherkin-rules/branches/main/features/IFC105_Resource-entities-need-to-be-referenced-by-rooted-entity.html) rooted-entity requirement and eliminates the validation service warning.

Figure 10.6.1.5-1 shows the resulting entity structure: the placeholder alignment's aggregation to the project, and its shape representation chain down to the tagged offset curve.

![Figure 10.6.1.5-1 — Entity relationship diagram: IfcProject aggregates a placeholder IfcAlignment ("A-line"), whose shape representation carries a tagged IfcOffsetCurveByDistances; the surface's Directrix references the same basis curve, and its cross-section Tags informally match the offset curve's Tag.](images/Figure_10.6.1.5-1_Placeholder_Alignment_Guide_Curve_ERD.svg)

*Figure 10.6.1.5-1 — Guide curve rootedness via placeholder `IfcAlignment`. `B-line` and `C-line` follow the same pattern (omitted for clarity).*

Because all three guide curves in this example share the same `BasisCurve` as the surface `Directrix`, tag matching is scoped to that shared basis curve. Tags `"A"`, `"B"`, and `"C"` need only be unique within the set of guide curves on `Main-Line` — they do not need to be globally unique across the model. The IFC specification does not state this scoping rule explicitly, but it follows from the interpretation discussed in §10.5.1.

The expected geometry is the same as that shown in Figure 10.6.1.4-1.

§10.6.2.5 demonstrates a contrasting rooting technique — a sibling `IfcShapeRepresentation` requiring no placeholder host entity at all — applied to the `IfcSectionedSolidHorizontal` counterpart of this example; see Figure 10.6.2.5-1 for comparison.

#### 10.6.1.6 String Lines — Independent Guide Curves

The file [`IfcSectionedSurface_with_stringlines_independent_guide_curves.ifc`](examples/IfcSectionedSurface_with_stringlines_independent_guide_curves.ifc) takes a different approach: the edge string lines are derived from independent `IfcAlignment` instances whose geometry is circular arcs, rather than parametric offsets from the centerline. `Main-Line` is a 200 m straight alignment. `Left_Edge` and `Right_Edge` are 150 m circular arcs (radius 300 m) starting at ±30 m from the centerline. The guide curves for tags `"A"` and `"C"` are zero-offset curves from these arc alignments; the guide curve for tag `"B"` is a zero-offset from `Main-Line`. As previously discussed, the alignments themselves cannot carry tags, so zero-offset `IfcOffsetCurveByDistances` instances are used.

Because the guide curves for tags `"A"` and `"C"` have a different `BasisCurve` than the surface `Directrix`, the Local Scoping rule from §10.5.1 no longer provides a natural disambiguation boundary. There is no shared parameterization to define which guide curves belong to which surface or solid, so tags must be treated as globally unique across the entire model. The IFC specification is silent on this distinction — it neither requires global uniqueness nor defines when surface-scoped uniqueness is sufficient.

This configuration is the most conceptually faithful to the idea of string lines — each edge path is an independent geometric entity — but it exposes two further specification gaps. First, `Left_Edge` and `Right_Edge` are 150 m long while the surface directrix is 200 m long. The guide curves' parameterization ends before the surface does, and the IFC specification's implicit extension rule (the last offset value continues to the end of the basis curve) does not resolve the inconsistency because the guide curves' basis curves are different from the surface directrix. Second, the cross-section at distance 200 m authors a width of 60 m each side, but the circular arc guide curves would position the edge vertices at a different lateral distance at that location. The specification does not state which governs — the authored section or the guide curve.

Figure 10.6.1.6-1 shows a plan view of the example string line model. The specification ambiguities identified above leave the correct output geometry undefined, and no implementation producing a verifiable result has been found.

![Figure 10.6.1.6-1 — Plan view showing the ambiguous geometry that arises when an IfcSectionedSurface has cross-sections perpendicular to the directrix but edge string line guide curves that diverge from that orientation. The specification does not prescribe which geometry governs where they disagree.](images/Figure_10.6.1.6_IfcSectionedSurface_with_ambiguous_geometry.png)

*Figure 10.6.1.6-1 — Plan view of `IfcSectionedSurface_with_stringlines_independent_guide_curves.ifc`. The centerline (`Main-Line`) and arc edge alignments (`Left_Edge`, `Right_Edge`) are shown; the surface geometry between them is indeterminate due to the specification gaps described above.*

Figure 10.6.1.6-2 shows the expected geometry of this example.

![Figure 10.6.1.6-2 — Synthesized 3D render showing the expected correct geometry of an IfcSectionedSurface with independent string line edge alignments: the surface curves smoothly to follow both guide curves simultaneously. No viewer capable of this interpretation was available at the time of writing.](images/Figure_10.6.1.6-2_IfcSectionedSurface_with_stringlines_expected_geometry.png)

*Figure 10.6.1.6-2 — Expected geometry of `IfcSectionedSurface_with_stringlines_independent_guide_curves.ifc`. This rendering has been synthesized to illustrate the correct geometry; no viewer capable of resolving string line guide curves has been found at the time of writing.*

#### 10.6.1.7 String Lines — Dense Section Approximation
...
Figure 10.6.1.7-2 zooms in on the far-right corner of the surface, where the gap between the exact circular arc guide curve (shown in orange) and the piecewise-linear surface edge is most visible. With only five sections, the approximation error is perceptible at the scale of the model.
...
The base cross-section is a 60 m wide trapezoid — vertices `(-30, 0)`, `(30, 0)`, `(29.5, 0.5)`, `(-29.5, 0.5)`, encoded as an `IfcArbitraryClosedProfileDef` — exaggerated in width relative to §10.6.2.1's rail ballast bed to make the rotation clearly visible. A single instance of this base profile is shared by both cross-section positions; each position wraps it in its own `IfcDerivedProfileDef` whose `IfcCartesianTransformationOperator2D` carries the rotation.
...
- [`IfcSectionedSolidHorizontal_retaining_wall_explicit_axis.ifc`](examples/IfcSectionedSolidHorizontal_retaining_wall_explicit_axis.ifc) — `Axis = (0, 0, 1)` supplied explicitly at each `CrossSectionPosition`; cross-section faces are plumb (Figure 10.5.5-1).
- [`IfcSectionedSolidHorizontal_retaining_wall_implicit_axis.ifc`](examples/IfcSectionedSolidHorizontal_retaining_wall_implicit_axis.ifc) — `Axis` omitted; the resulting orientation is implementation-defined (Figure 10.5.5-2).

The alignment is a 50 m straight grade at 10% due East. Three cross-sections are defined — at 0 m, 25 m, and 50 m — and the geometry engine interpolates linearly between them. The cross-section is a retaining wall with footing; the stem height varies from 2.0 m at the start, to 3.0 m at the midpoint, and back to 1.5 m at the end. The footing width also varies: 2.0 m at the start and end, widening to 2.5 m at the midpoint, while the toe and wall thickness remain constant throughout. See §10.5.5 for a discussion of the `Axis` direction and its geometric consequences.

#### 10.6.2.5 String Lines — Guide Curves as Representations

The file [`IfcSectionedSolidHorizontal_with_stringlines_guide_curves_as_alignments.ifc`](examples/IfcSectionedSolidHorizontal_with_stringlines_guide_curves_as_alignments.ifc) demonstrates guide curves controlling tagged vertices of an `IfcArbitraryClosedProfileDef`, replacing the `IfcOpenCrossProfileDef.Tags` mechanism of the surface with `IfcCartesianPointList2D.TagList`. It is the solid counterpart to [`IfcSectionedSurface_with_stringlines_guide_curves_as_alignments.ifc`](examples/IfcSectionedSurface_with_stringlines_guide_curves_as_alignments.ifc) (§10.6.1.5) in every respect **except** the specific technique used to satisfy the IFC105 rooted-entity requirement of §10.5.7: that example uses a placeholder `IfcAlignment`; this one uses a sibling representation of the same `IfcPavement`. The two files are presented side by side for comparison — they produce identical guide curve geometry through two different rooting techniques.

The directrix is the same 200 m straight alignment (`Main-Line`) at flat grade. Two cross-section positions are authored — one at each end. Six guide curves — bare `IfcOffsetCurveByDistances` instances tagged `"A_top"`, `"B_top"`, `"C_top"`, `"A_bot"`, `"B_bot"`, `"C_bot"` — share `Main-Line` as their `BasisCurve`, the same curve as the solid `Directrix`; because all six share that basis curve, tag matching is scoped to that shared parameterization (see §10.5.1). The top guide curve offsets are unchanged from §10.6.1.5's `A`/`B`/`C`: `A_top` and `C_top` widen from ±30 m to ±45 m at the 100 m midpoint and return; `B_top` holds the crown at zero offset throughout. The soffit guide curves (`A_bot`, `B_bot`, `C_bot`) follow the same lateral path as their top counterparts, offset a further 1 m downward to hold the slab thickness constant.

Rather than wrapping each guide curve in its own placeholder `IfcAlignment`, the six tagged `IfcOffsetCurveByDistances` instances are collected into a second `IfcShapeRepresentation` — `RepresentationIdentifier = "Axis"`, `RepresentationType = "GeometricCurveSet"` — that is a sibling entry, alongside the `"Body"`/`"SolidModel"` representation holding the `IfcSectionedSolidHorizontal`, in the same `IfcPavement`'s `IfcProductDefinitionShape.Representations` list. Because that `IfcProductDefinitionShape` is only ever reachable through the rooted `IfcPavement`, the guide curves inherit a traceable path to a rooted entity without any dedicated placeholder host — see §10.5.7 for the general technique and its trade-offs against the placeholder-alignment and `IfcAnnotation` approaches. Figure 10.6.2.5-1 illustrates the resulting entity structure; contrast it with Figure 10.6.1.5-1, which shows the placeholder-alignment structure used by the surface counterpart.

![Figure 10.6.2.5-1 — Entity relationship diagram: IfcPavement's IfcProductDefinitionShape carries two sibling IfcShapeRepresentation entries — "Body" (SolidModel, containing the IfcSectionedSolidHorizontal) and "Axis" (GeometricCurveSet, containing the six tagged IfcOffsetCurveByDistances guide curves) — with no placeholder IfcAlignment host entity.](images/Figure_10.6.2.5-1_Sibling_Representation_Guide_Curve_ERD.svg)

*Figure 10.6.2.5-1 — Guide curve rootedness via sibling representation: the solid and its six guide curves hang off the same `IfcProductDefinitionShape`.*

The closed cross-section is a V-shaped pavement slab 60 m wide (nominal) and 1 m thick, with a 20% crown slope. Tags are assigned through `IfcCartesianPointList2D.TagList`: all six vertices carry matching guide curve tags — `"A_top"`, `"B_top"`, `"C_top"` on the top surface and `"A_bot"`, `"B_bot"`, `"C_bot"` on the soffit — so every vertex, top and bottom, follows its own guide curve.

**Key difference from the surface counterpart.** In `IfcOpenCrossProfileDef`, the `Tags` attribute is a list of vertex labels on the open profile. In `IfcArbitraryClosedProfileDef`, there is no `Tags` attribute; vertex labels are carried by `IfcCartesianPointList2D.TagList`. Both attributes achieve the same match — an offset curve's `Tag` is compared against the vertex label, and matching vertices are guided. A vertex with a tag that matches no guide curve is treated as unguided — its position is linearly interpolated between its authored positions at the `CrossSectionPositions` — though the specification does not explicitly confirm this behavior (see §10.5.2).

#### 10.6.2.6 String Lines — Independent Guide Curves

The file [`IfcSectionedSolidHorizontal_with_stringlines_independent_guide_curves.ifc`](examples/IfcSectionedSolidHorizontal_with_stringlines_independent_guide_curves.ifc) is the solid counterpart to [`IfcSectionedSurface_with_stringlines_independent_guide_curves.ifc`](examples/IfcSectionedSurface_with_stringlines_independent_guide_curves.ifc) (§10.6.1.6). It demonstrates `IfcSectionedSolidHorizontal` where the edge guide curves are derived from independent `IfcAlignment` instances with circular arc geometry, rather than parametric offsets from the centerline directrix. The overall structure mirrors §10.6.2.5, but the guide curves for tags `"A_top"` and `"C_top"` have a different `BasisCurve` than the solid `Directrix`. As in §10.6.2.5, the tag-carrying guide curves are rooted via a sibling `"Axis"` representation rather than placeholder alignments; see that section (and §10.5.7) for the technique and its rationale. This section's focus remains the independent-`BasisCurve` edge geometry, which is unchanged from §10.6.1.6.
...
Both edge alignments carry a constant-grade vertical layout with a start height of $-\frac{1}{2} \times 30 \times 0.2 = -3\ \mathrm{m}$, placing each shoulder 3 m below the crown at the start. Unlike the surface counterpart (§10.6.1.6), the tag-carrying guide curves here are not wrapped in placeholder `IfcAlignment` instances: `A_top-line`, `B_top-line`, and `C_top-line` are bare, zero-offset `IfcOffsetCurveByDistances` entities — `A_top-line` referencing `Left_Edge` as its `BasisCurve`, `B_top-line` referencing `Main-Line`, and `C_top-line` referencing `Right_Edge`. `Left_Edge` and `Right_Edge` themselves remain ordinary `IfcAlignment` instances with real circular-arc geometry, unchanged from the surface example. The tags `"A_top"`, `"B_top"`, `"C_top"` are set directly on the offset curves, which — together with §10.6.2.5's guide curves — are placed in a sibling `"Axis"` `IfcShapeRepresentation` to satisfy the IFC105 rootedness requirement without a placeholder host entity (see §10.5.7). This tag naming matches §10.6.2.5's scheme — `_top` for guided vertices, `_bot` for the soffit — even though only the top vertices have guide curves in this example.

Because the guide curves for tags `"A_top"` and `"C_top"` have a `BasisCurve` independent of the solid `Directrix`, the Local Scoping rule from §10.5.1 no longer provides a natural disambiguation boundary. Tags must therefore be treated as globally unique across the entire model. The `"A_bot"`, `"B_bot"`, and `"C_bot"` tags on the bottom vertices serve as distinct identifiers that match no guide curve, ensuring the soffit falls back to linear interpolation.
