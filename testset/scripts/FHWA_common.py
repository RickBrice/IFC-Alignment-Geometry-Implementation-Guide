...
#
# Shared alignment-building code for the FHWA Bridge Geometry Manual Appendix B
# "E-Line" example, used by Build_FHWA_Alignment.py, Build_FHWA_Alignment_with_Linear_Placement.py,
# and Build_FHWA_Alignment_LinearPlacement_VerticalOffset.py. Kept separate from common.py
# because this module is specific to the FHWA E-Line example rather than generic across
# the whole testset pipeline.
import math
...
def setup_eline_project(project_name="FHWA Alignment"):
    """Create the IFC file, project, site, units (international foot + radian), and the
    empty E-Line alignment (horizontal + vertical layouts, start station 100+00).

    Returns (ifc, alignment).
    """
    ifc = ifcopenshell.file(schema="IFC4X3_ADD2")
    ifc.header.file_description.description = ["ViewDefinition [Alignment-basedView]"]

    ifcopenshell.api.root.create_entity(ifc, ifc_class="IfcProject", name=project_name)
    ...
    alignment = ifcopenshell.api.alignment.create(
        ifc, ALIGNMENT_NAME, include_vertical=True, include_geometry=False,
...
def build_eline_layout(ifc, alignment):
    """Build the E-Line horizontal (7 segments) and vertical (9 segments) layouts,
    then create the geometric representation (IfcGradientCurve) and its end point.
    """
