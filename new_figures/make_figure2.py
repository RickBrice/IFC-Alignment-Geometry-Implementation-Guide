...
# Figure 2: OffsetVertical is defined as plain global Z (straight up),
# NOT perpendicular to the tangent. Contrast with Figure 1.
Yv = Zg.copy()                  # -> OffsetVertical direction (global Z)
...
# OffsetLateral is unchanged from Figure 1: it has zero Z-component by
# construction, i.e. it already lies parallel to the global XY plane.
...
# Longitudinal direction -> OffsetLongitudinal
# Figure 2: uses Th (horizontal projection of the tangent), NOT the full
# 3D tangent T, so OffsetLongitudinal stays parallel to global XY. This
# keeps Longitudinal/Lateral/Vertical mutually orthogonal given that
# OffsetVertical here is global Z (T itself is tilted by the grade and
# would not be orthogonal to global Z).
draw_arrow(P, Th, '#c53030', 'OffsetLongitudinal\n(horizontal, \u2225 global XY)', length=arrow_len,
           txt_off=(0, 0, -3), txt_scale=1.0, ha='left', va='top')
...
# Vertical direction: plain global Z -> OffsetVertical
draw_arrow(P, Yv, '#2b6cb0', 'OffsetVertical\n(global Z)', length=arrow_len,
           txt_off=(-3, 0, 0), txt_scale=1.0, ha='right', va='top')
...
#    is applied last -- here along Th (horizontal), not the full 3D
#    tangent T, so that Lateral/Vertical/Longitudinal remain mutually
#    orthogonal given OffsetVertical = global Z in this figure.
# ---------------------------------------------------------------
...
# Uses Th (horizontal), not T, to match the corrected OffsetLongitudinal direction.
Q = draw_offset_segment(Pb, offset_longitudinal * Th, '#c53030')

# Q marker drawn later as a 2D overlay so it renders on top -- see section 7
Q_label_anchor = Q + np.array([5, 5, -5]) + np.array([0, 0, 7])
...
ax.set_title('Proposed offset directions for IfcPointByDistanceExpression', pad=20)
...
plt.tight_layout()
plt.savefig('/mnt/user-data/outputs/ifc_offset_directions_fig2.svg', dpi=200, bbox_inches='tight')
print("saved")
