...
ax.text(P[0] - 10, P[1] + 6, P[2] + 4, 'P', fontsize=12, ha='center',
        fontweight='bold', bbox=dict(facecolor='white', edgecolor='none', alpha=0.8, pad=1))
...
# RefDirection (local X axis) defaults to the curve tangent at Location
# per 8.9.3.4.3: "If RefDirection is omitted, the direction is taken
# from the curve tangent at Location."
draw_arrow(P, T, '#c53030', 'RefDirection (local X)\n= curve tangent T', length=arrow_len,
           txt_off=(0, 0, 0), txt_scale=1.0, ha='left', va='bottom')

# Derived local Y axis = Axis (Z) x RefDirection (X). Anchor exactly at
# the arrowhead tip, with the label's top-right bbox corner pinned there.
draw_arrow(P, Lat, '#2f855a', 'Derived local Y\n= Axis \u00d7 RefDirection', length=arrow_len,
           txt_off=(0, 0, 0), txt_scale=1.0, ha='right', va='top')

# Axis (local Z axis), consistent with Figure 1's OffsetVertical: the
# consensus reading is that it is perpendicular to the tangent, within
# the vertical plane containing the tangent -- NOT plain global Z.
draw_arrow(P, Yv, '#2b6cb0', 'Axis (local Z)\n(\u22a5 to T, in vertical plane)', length=arrow_len,
...
ax.set_title('Figure 3 \u2014 Default Axis and RefDirection for IfcAxis2PlacementLinear', pad=20)
...
plt.tight_layout()
plt.savefig('/mnt/user-data/outputs/ifc_axis2placementlinear_defaults_fig3.svg', dpi=200, bbox_inches='tight')
print("saved")
